...
class DBSCANAngular:
    ...
    # ------------------------------------------------------------------
    # DBSCAN angular distance matrix
    # ------------------------------------------------------------------

    def _build_angular_distance_matrix(self, X):
        """
        Compute the N×N pairwise angular-direction distance matrix.

        The metric is the absolute difference of arctan2 angles (direction
        from origin), wrapped to [0, π].
        """
        angles = np.arctan2(X[:, 1], X[:, 0])          # (N,)
        diff = np.abs(angles[:, None] - angles[None, :])  # (N, N)
    ...
    # ------------------------------------------------------------------
    # Core fit  (DBSCAN / HDBSCAN backends + shared post-processing)
    # ------------------------------------------------------------------
    ...
    def _fit_dbscan(self, X):
        """Run DBSCAN on the precomputed angular distance matrix."""
        dist_matrix = self._build_angular_distance_matrix(X)
        db = DBSCAN(
            eps=self.eps,
    ...
    def _fit_hdbscan(self, X):
        """
        Run HDBSCAN on the precomputed angular distance matrix.
    ...
    def fit(self, X):
        ...
        classic DBSCAN with a precomputed angular distance matrix.
